...
function Line() {
  return <div className="h-1 flex-1 bg-yellow-100"></div>;
}
...
export default function ScoreLine({ currentScore, maxScore }) {
  ...
  return (
    <section
      className={clsx("h-8 flex items-center", { "animate-pulse": isLoading })}
    >
      <div
        className={clsx("flex-1 flex justify-between items-center relative")}
      >
        <div
          style={{ width: `${currentPercentage}%` }}
          className="h-1 bg-yellow-200 absolute transition-[width] duration-250"
        ></div>
        <p
          style={{ left: `${currentPercentage}%` }}
          className={clsx(
            "rounded-full size-8 border-2 text-xs flex justify-center items-center border-yellow-300 bg-white absolute z-20 transition-[left] duration-250",
